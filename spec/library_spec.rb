...
RSpec.describe Library do
  ...
  describe '#adds_track' do
    it 'adds a track to its storage' do
      l = Library.new
      response = l.add_track('Ghostface Killah', 'One')

      expect(l.tracks.count).to eq(1)
      expect(l.tracks.first.artist).to eq(response.artist)
      expect(l.tracks.first.title).to eq(response.title)
    end

    it 'returns false if track with title already exists' do
      response = library.add_track('Nonsense Artist', 'One')

      expect(library.tracks.count).to eq(1)
